...
DATABASE = "sasquatch"
import re
from flask_app.models import sasquatch_model
...
#    add your getbyid once you have exam
    @classmethod 
    def get_by_id(cls,data):
        query = "SELECT * FROM users LEFT JOIN sasquatch ON users.id = sasquatch.user_id WHERE users.id = %(id)s;"
        results = connectToMySQL(DATABASE).query_db(query,data)
        user = cls(results[0])
        list_of_sasquatch = []
        for row in results:
            if row['sasquatch.id'] == None:
                break 
            sasquatch_data = {
                **row,
                'id': row['sasquatch.id'],
                'created_at': row['sasquatch.created_at'],
                'updated_at': row['sasquatch.updated_at']
            }  
            this_sasquatch =  sasquatch_model.Sasquatch(sasquatch_data)   
            list_of_sasquatch.append(this_sasquatch)
            user.sasquatch = list_of_sasquatch
        return user
